...
/**
 * This class and its inner classes are only used as a work-around for the J2SSH configuration
 * framework. We do not want to provide the expected xml-files, we just want programmatical control
 * over the configuration and feed it values from our own module configuration file.
 *
 * @author Henrik Schroder
 */
class DummyConfigurationContext implements ConfigurationContext {
  private final HashMap<Class, Object> configurations = new HashMap<Class, Object>();

  private final String serverHostKeyFile;

  private final int port;

  private final int maxAuthRetry;

  private final int maxConn;

  public DummyConfigurationContext(
          final String serverHostKeyFile, final int port, final int maxAuthRetry, final int maxConn) {
  ...
  public boolean isConfigurationAvailable(final Class cls) {
    return true;
  }

  public Object getConfiguration(final Class cls) throws ConfigurationException {
  ...
  public void initialize() throws ConfigurationException {
    try {
      final ServerConfiguration fake1 =
          new DummyServerConfiguration(serverHostKeyFile, port, maxAuthRetry, maxConn);
      configurations.put(ServerConfiguration.class, fake1);
      final PlatformConfiguration fake2 = new DummyPlatformConfiguration();
  ...
  private static class DummyPlatformConfiguration extends PlatformConfiguration {
    public DummyPlatformConfiguration()
    ...
    public String getSetting(final String name, final String defaultValue) {
      return defaultValue;
    }
  ...
  private static class DummyServerConfiguration extends ServerConfiguration {
    private final Map<String, SshPrivateKey> serverHostKeys = new HashMap<String, SshPrivateKey>();
    ...
    public DummyServerConfiguration(
            String serverHostKeyFile, final int port, final int maxAuthRetry, final int maxConn)
        throws SAXException, ParserConfigurationException, IOException {
      super(null);
      ...
      if (!f.exists()) {
        serverHostKeyFile = ConfigurationLoader.getConfigurationDirectory() + serverHostKeyFile;
      ...
      try {
        if (f.exists()) {
          ...
          Logger.warn(this, "Private key file '" + serverHostKeyFile + "' could not be found");
        }
      } catch (final InvalidSshKeyException ex) {
        Logger.warn(this, "Failed to load private key '" + serverHostKeyFile, ex);
      } catch (final IOException ioe) {
        Logger.warn(this, "Failed to load private key '" + serverHostKeyFile, ioe);
      }
    }
