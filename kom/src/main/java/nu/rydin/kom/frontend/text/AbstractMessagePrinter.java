...
public abstract class AbstractMessagePrinter implements MessagePrinter {
  ...
  public final void printMessage(Context context, Envelope envelope) throws KOMException {
  ...
  /**
   * Prints the first line of the message, containing text number, author and date.
   *
   * @param context
   * @param envelope
  ...
  /**
   * This method decides how the text number should look like and what parts to print.
   *
   * @param context
   * @param envelope
   * @return
   ...
   * @throws UnexpectedException
   */
  protected abstract StringBuffer getFormattedMessageId(Context context, Envelope envelope)
      throws ObjectNotFoundException, UnexpectedException;

  /**
   * This method is supposed to print information about mail, typically stored as Message
   * Attributes.
   *
   * @param context
   * @param envelope
   ...
   */
  protected abstract void printMailInformation(Context context, Envelope envelope)
    ...
    if (replyTo != null) {
      ...
      try {
        attrs =
            context
                .getSession()
                .getMatchingMessageAttributes(
                    envelope.getMessage().getId(), MessageAttributes.COMMENT_TYPE);
      } catch (UnexpectedException e) {
        // Quietly ignore.
      }
      PrintUtils.printIndented(
          out,
          getFormattedOriginInfo(
              context, replyTo, (attrs != null && attrs.length > 0) ? attrs : null),
          width,
          0);
    } else {
      ...
      for (int idx = 0; idx < attributes.length; ++idx) {
        ...
        if (each.getKind() == MessageAttributes.ORIGINAL_DELETED) {
          PrintUtils.printIndented(out, getFormattedOriginDeletedInfo(context, each), width, 0);
        }
      }
    }
  }
   ...
   * @return
   */
  protected abstract String getFormattedOriginInfo(
      Context context, RelatedMessage replyTo, MessageAttribute[] attrs);

  /**
   * Returns a string with information about the origin text (which is deleted now).
   *
   * @param context
   * @param originalDeletedAttribute
   * @return
   */
  protected abstract String getFormattedOriginDeletedInfo(
      Context context, MessageAttribute originalDeletedAttribute);

  /**
   * This method is supposed to print information about the receiving conferences of this text. See
   * also AbstractMessagePrinter.getFooterReceivers which does the equivalent in the footer.
   *
   * @param context
   * @param envelope
   ...
   * @throws UnexpectedException
   */
  protected abstract void printHeaderReceivers(Context context, Envelope envelope)
  ...
  protected void printSubject(Context context, Envelope envelope) {
    ...
    PrintUtils.printRepeated(
        out, '-', Math.min(width - 1, subjLine.length() + message.getSubject().length()));
    out.println();
  }

  /**
   * Prints information about the author of the text (if enabled by the SHOW_TEXT_FOOTER flag).
   *
   * @param context
   * @param envelope
   * @throws ObjectNotFoundException
  ...
  protected void printFootNotes(Context context, Envelope envelope) {
    ...
    for (int idx = 0; idx < attributes.length; ++idx) {
      ...
      if (each.getKind() == MessageAttributes.FOOTNOTE) {
        ...
        out.print(label);
        dc.messageBody();
        PrintUtils.printIndented(out, each.getValue(), width - label.length(), 0, label.length());
      }
    }
  }
  ...
  /**
   * This method is supposed to print information about the receiving conferences of this text. See
   * also AbstractMessagePrinter.getHeaderReceivers which does the equivalent in the header.
   *
   * @param context
   * @param envelope
   * @throws AuthorizationException
   ...
   * @throws UnexpectedException
   */
  protected abstract void printFooterReceivers(Context context, Envelope envelope)
      throws AuthorizationException, ObjectNotFoundException, UnexpectedException;
    ...
    for (int idx = 0; idx < attributes.length; ++idx) {
      ...
      if (each.getKind() == MessageAttributes.NOCOMMENT) {
        PrintUtils.printIndented(
            out,
            formatter.format(
                getResourceKey("nocomment"),
                context.formatObjectName(each.getUsername(), each.getUserId())),
            width,
            0);
      }
    }
  ...
  /**
   * Prints information about the replies to this text. The actual layout of each reply is decided
   * in the getFormattedReply method.
   *
   * @param context
   * @param envelope
   */
    ...
    for (int idx = 0; idx < replies.length; ++idx) {
      ...
      try {
        ma =
            context
                .getSession()
                .getMatchingMessageAttributes(occ.getGlobalId(), MessageAttributes.COMMENT_TYPE);
        hasAttr = (ma.length > 0);
      } catch (UnexpectedException e) {
        // Never mind.
      }
      ...
      PrintUtils.printIndented(
          out, getFormattedReply(context, each, occ, hasAttr ? ma : null), width, 0);
    }
  }
   ...
   * @return
   */
  protected abstract String getFormattedReply(
      Context context, RelatedMessage reply, MessageOccurrence occ, MessageAttribute[] attrs);

  /**
   * Finds the MOVEDFROM attribute and returns and returns it if found. Otherwise null.
   *
   * @param attributes
   * @return
   */
   ...
   *
   * @param keyName
   * @return a resource key that should match an entry in the messages.properties file.
   */
  ...
  /**
   * Determines if the envelope contains a mail, in this case defines as having an message
   * occurrence in the current user's mailbox.
   *
   * @param context
   * @param envelope
   * @return
