...
public class Copy extends AbstractCommand {
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    if (messageid == -1) throw new NoCurrentMessageException();

    // ConferenceInfo ci = session.getCurrentConference();
    ConferenceInfo ci =
        session.getConference(session.getCurrentMessageOccurrence().getConference());
    boolean mustConfirm =
        ((ci.getPermissions() & ConferencePermissions.READ_PERMISSION) == 0)
            || (ci.getVisibility() != Visibilities.PUBLIC);
    if (mustConfirm) {
      LineEditor in = context.getIn();
      int choice =
          in.getChoice(
              fmt.format("copy.confirm.leak", ci.getName().getName())
                  + " ("
                  + fmt.format("misc.yes")
                  + "/"
                  + fmt.format("misc.no")
                  + ")? ",
              new String[] {fmt.format("misc.no"), fmt.format("misc.yes")},
              -1,
