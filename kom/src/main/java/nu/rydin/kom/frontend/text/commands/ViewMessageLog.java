...
public abstract class ViewMessageLog extends AbstractCommand {
  ...
  public void innerExecute(Context context, Object[] parameterArray) throws KOMException {

    PrintWriter out = context.getOut();
    MessageFormatter formatter = context.getMessageFormatter();
    int num = -1;
    if (parameterArray[0] == null) num = 10;
    else {
    ...
    for (int idx = top - 1; idx >= 0; --idx) {
      ...
      if (each.isSent()) {
        ...
        for (int recIdx = 0; recIdx < nRecs; ++recIdx) {
          ...
          if (recIdx < nRecs - 1) sb.append(", ");
        }
      } else {
        if (kind == MessageLogKinds.CHAT) sb.append("< ");
        sb.append(each.getAuthorName());
      }
      if (kind != MessageLogKinds.CONDENSED_BROADCAST) sb.append(':');
      sb.append(' ');
  ...
  protected abstract MessageLogItem[] fetch(ServerSession session, int num) throws KOMException;
}
