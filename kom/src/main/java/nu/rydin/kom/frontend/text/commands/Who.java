...
public class Who extends AbstractCommand {
  ...
  public void execute(Context context, Object[] parameterArray) throws KOMException {
    ...
    //
    if (context.getCachedUserInfo().testFlags(0, UserFlags.COMPACT_WHO)) this.makeCompact(users);
    ...
    for (int idx = 0; idx < top; ++idx) {
      ...
      if (each == null) continue; // Eliminated by compacting
      String confName =
          each.isInMailbox()
      ...
      PrintUtils.printRightJustified(
          out, StringUtils.formatElapsedTime(now - each.getLoginTime()), 7);
      ...
      switch (each.getActivity()) {
        case Activities.AUTO: // OpenKOM standard, shows conference.
          PrintUtils.printIndented(
              out,
              formatter.format(
                  "who.format", new Object[] {context.formatObjectName(each.getUser()), confName}),
              lastColWidth,
              0,
              firstColsWidth);
          break;
        ...
        case Activities.MAIL: // User is writing a mail
          PrintUtils.printIndented(
              out,
              formatter.format(
                  each.getLastObject() == context.getSession().getLoggedInUserId()
                      ? "who.mail.to.you"
                      : "who.mail",
                  new Object[] {context.formatObjectName(each.getUser())}),
              lastColWidth,
              0,
              firstColsWidth);
          break;
        ...
        case Activities.POST: // User is writing a post or a reply
          PrintUtils.printIndented(
              ...
              formatter.format(
                  "who.post.where",
                  new Object[] {context.formatObjectName(each.getUser()), confName}),
              lastColWidth,
              0,
              firstColsWidth);
          break;
        ...
        case Activities.BROADCAST: // User is sending a broadcast message
          PrintUtils.printIndented(
              out,
              formatter.format(
                  "who.broadcast", new Object[] {context.formatObjectName(each.getUser())}),
              lastColWidth,
              0,
              firstColsWidth);
          break;
        ...
        case Activities.FREETEXT: // User has set his or her own activity
          PrintUtils.printIndented(
              ...
              formatter.format(
                  "who.freeformat",
                  new Object[] {context.formatObjectName(each.getUser()), each.getActivityText()}),
              lastColWidth,
              0,
              firstColsWidth);
          break;
        ...
        default: // Can't happen. We think.
          PrintUtils.printIndented(out, "Strange state.", lastColWidth, 0, firstColsWidth);
          break;
      }
      ;
      if (idle < 60000) ++active;
    }
    out.println();
    out.println(
        formatter.format("who.total", new Object[] {new Integer(top), new Integer(active)}));
  }
