...
public class ChangeUnread extends AbstractCommand {
  public ChangeUnread(Context context, String fullName, long permissions) {
    super(
        ...
        new CommandLineParameter[] {
          new IntegerParameter("change.unread.param.0.ask", true),
          new ConferenceWildcardParameter(false)
        },
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray) throws KOMException, IOException {
    ...
    if (null == parameterArray[1]) {
      ...
      if (-1 != conf) ss.changeUnread(number, conf);
      else ss.changeUnreadInAllConfs(number);
    }
