...
public class ChangeConferenceParameters extends AbstractCommand {
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    //
    ConferenceType ct =
        ConferenceUtils.askForConferenceType(
            context,
            ci.getPermissions(),
            ci.getNonmemberPermissions(),
            conference.getName().getVisibility());

    // Update conference
    //
    session.updateConferencePermissions(
        conference.getId(), ct.getPermissions(), ct.getNonMemberPermissions(), ct.getVisibility());
    ...
    context
        .getOut()
        .println(
            fmt.format("change.conference.parameters.confirm", conference.getName().toString()));
  }
}
