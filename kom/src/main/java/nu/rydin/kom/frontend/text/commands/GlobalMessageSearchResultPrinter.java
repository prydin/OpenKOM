...
public class GlobalMessageSearchResultPrinter implements MessageSearchResultPrinter {
  ...
  public void printSearchResultRow(Context context, KOMWriter out, MessageSearchResult msr) {
    ...
    PrintUtils.printRightJustified(
        out, "(" + String.valueOf(gmsr.getGlobalId()) + ")", GLOBALID_COL_WIDTH);
    out.print(" ");
    PrintUtils.printRightJustified(out, String.valueOf(gmsr.getLocalId()), LOCALID_COL_WIDTH);
    out.print(" ");
    dc.output();
    PrintUtils.printLeftJustified(
        out, context.formatObjectName(gmsr.getConference()), CONFERENCE_COL_WIDTH);
    out.print(" ");
    dc.header();
    out.print(msr.getReplyTo() > 0 ? "  " : "* ");
    dc.output();
    PrintUtils.printLeftJustified(out, gmsr.getSubject(), SUBJECT_COL_WIDTH);
    out.println();
    out.flush();
  }
