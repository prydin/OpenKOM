...
public class CreateConference extends AbstractCommand {
  public CreateConference(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new RawParameter("create.conference.param.0.ask", true)},
        permissions);
  }
    ...
    try {
      context
          ...
          .createConference(
              ...
              keywords,
              ct.getPermissions(),
              ct.getNonMemberPermissions(),
              ct.getVisibility(),
              replyConf);
    } catch (DuplicateNameException e) {
      out.println(
          context.getMessageFormatter().format("create.conference.ambiguous", e.getMessage()));
    }
  }
}
