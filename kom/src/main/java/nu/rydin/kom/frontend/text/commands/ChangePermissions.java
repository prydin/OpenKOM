...
public class ChangePermissions extends AbstractCommand {
  public ChangePermissions(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new UserParameter("change.permissions.user.question", true)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    out.println(
        formatter.format(
            "change.permissions.header",
            new Object[] {
              userAssoc.getName().getName(), session.getCurrentConference().getName()
            }));
    out.println();
    ...
    permissions |=
        this.askForPermission(
            context, "permission.read", ConferencePermissions.READ_PERMISSION, old);
    permissions |=
        this.askForPermission(
            context, "permission.write", ConferencePermissions.WRITE_PERMISSION, old);
    permissions |=
        this.askForPermission(
            context, "permission.reply", ConferencePermissions.REPLY_PERMISSION, old);
    permissions |=
        this.askForPermission(
            context, "permission.delete", ConferencePermissions.DELETE_PERMISSION, old);
    permissions |=
        this.askForPermission(
            context, "permission.administrator", ConferencePermissions.ADMIN_PERMISSION, old);
      ...
      try {
        ...
        int choice =
            context
                .getIn()
                .getChoice(
                    prefix + " " + formatter.format(format) + ": ",
                    new String[] {formatter.format("misc.no"), formatter.format("misc.yes")},
                    defaultChoice,
                    error);
