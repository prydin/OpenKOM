...
public class FullscreenEditor extends EditorContext {
  ...
  /** */
  public FullscreenEditor(Context context) throws IOException, UnexpectedException {
    super(context);
    ...
    try {
      ...
      for (; ; ) {
        ...
        try {
          ...
          //
          if (m_cy >= buffer.size()) buffer.add("");
          ...
          if (m_cy + m_viewportStart > 0) flags |= LineEditor.FLAG_STOP_ON_BOL;
          try {
  ...
  protected void deleteLine(int line, boolean moveUp) {
    ...
    if (line > 0) this.wordWrap(line - 1);

    // Move cursor up
    //
    if (line > 0 && moveUp) this.moveUp();
    m_cx = 0;
  ...
  protected int wordWrap(int line) {
    ...
    for (; ; ) {
      // Merge lines
      //
      if (line >= buffer.size() - 1) buffer.add("");
  ...
  protected void insertLine(String line) {
    ...
    //
    Buffer buffer = this.getBuffer();
    if (m_cy <= m_maxY - 1) this.scrollRegionDown(m_cy + 1, m_maxY, 1);
    else this.scrollRegionUp(0, m_maxY - 1, 1);
    m_tc.eraseToEndOfLine();

    m_cx = 0;
    if (m_cy + m_viewportStart >= buffer.size() - 1) buffer.add(line);
    else buffer.insertBefore(m_cy + m_viewportStart + 1, line);
  }
  ...
  protected void moveDown() {
    if (this.isAtEndOfBuffer()) return;
  ...
  protected void cursorUp(int n) {
    if (m_tc.canUp()) m_tc.up(n);
    else m_tc.setCursor(m_cy - n, 0);
  }

  protected void cursorDown(int n) {
    if (m_tc.canDown()) m_tc.down(n);
    else m_tc.setCursor(m_cy + n, 0);
  }
  ...
  public void revealCursor(boolean refresh) {
    ...
    m_cy -= d;
    m_viewportStart += d;
    if (refresh) this.refreshViewport();
  }
  ...
  protected void refreshRegion(int start, int end) {
    ...
    for (int idx = start; idx < top; ++idx) {
      ...
      m_tc.eraseLine();
      if (idx < buffer.size() - m_viewportStart) out.print(buffer.get(m_viewportStart + idx));
    }
    m_tc.setCursor(m_cy, m_cx);
    out.flush();
  }
