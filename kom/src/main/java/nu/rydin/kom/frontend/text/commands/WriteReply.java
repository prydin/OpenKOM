...
public class WriteReply extends AbstractCommand {
  public WriteReply(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new TextNumberParameter(false), new RawParameter("", false)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    if (0 != parmcount) {
      if (2 == parmcount) {
        ...
        if (null != parameterArray[0]) hasId = true;
        else hasType = true;
      }
    }
    ...
    try {
      originalMessage =
          session.getMostRelevantOccurrence(session.getCurrentConferenceId(), replyToId);
    } catch (ObjectNotFoundException e) {
      throw new MessageNotFoundException();
    }
    ...
    if (originalConference.getId() == context.getLoggedInUserId()) {
      ...
      context
          ...
          .println(
              context
                  ...
                  .format(
                      "write.mail.saved",
                      session.getUser(editor.getRecipient().getId()).getName()));
    } else {
      ...
      if ((context.getCachedUserInfo().getFlags1() & UserFlags.REPLY_IN_CURRENT_CONF) == 0
        ...
        //
        if (originalConference.getReplyConf() == -1) recipient = originalConference;
        else recipient = session.getConference(originalConference.getReplyConf());
      } else {
          ...
          .println(
              context
                  ...
                  .format(
                      "write.message.saved",
                      new Object[] {
                        new Integer(newMessage.getLocalnum()), new Long(newMessage.getGlobalId())
                      }));
    }
    ...
    //
    if (hasType) {
      session.addMessageAttribute(
          newMessage.getGlobalId(),
          MessageAttributes.COMMENT_TYPE,
          (String) parameterArray[1],
          false);
    }
  }
}
