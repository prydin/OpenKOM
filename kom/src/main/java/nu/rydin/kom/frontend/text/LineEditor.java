...
public class LineEditor implements NewlineListener {
    ...
    public void run() {
      try {
        ...
        for (; ; ) {
          ...
          if (e != null) LineEditor.this.handleEvent(e);
        }
  ...
  public LineEditor(
      ...
      KOMWriter out,
      EventTarget target,
      TerminalSettingsProvider tsProvider,
  ...
  public void setSession(ServerSession session) {
    if (m_session != null) throw new IllegalStateException("Already have a session!");
    m_session = session;
  ...
  public String readLineStopOnEvent()
    ...
    try {
      return innerReadLine(
          null, null, 0, FLAG_STOP_ON_EVENT | FLAG_ECHO | FLAG_STOP_ONLY_WHEN_EMPTY);
  ...
  public String readLineStopOnEvent(String defaultString)
    ...
    try {
      return innerReadLine(
          defaultString, null, 0, FLAG_STOP_ON_EVENT | FLAG_ECHO | FLAG_STOP_ONLY_WHEN_EMPTY);
    } catch (LineOverflowException e) {
      throw new RuntimeException("This should not happen!", e);
    } catch (LineUnderflowException e) {
      throw new RuntimeException("This should not happen!", e);
    ...
    for (; ; ) {
      ...
      if (tmp.length() > 0) {
        for (int idx = 0; idx < top; ++idx) {
          if (choices[idx].toUpperCase().startsWith(tmp)) return idx;
        }
      }
   ...
   */
  public boolean getMore(String prompt) throws IOException, InterruptedException {
    boolean result = false;
    ...
    try {
      if ((this.m_session.getUser(this.m_session.getLoggedInUserId()).getFlags1()
              & UserFlags.TREAT_SPACE_AS_NEWLINE)
          != 0) addSpaceAsYesChar = true;
    } catch (Exception e) {
   ...
   */
  public char waitForCharacter(char[] allowedCharacters) throws IOException, InterruptedException {
  ...
  protected char innerReadCharacter(int flags)
    ...
    while (true) {
      ...
      if (!(ev instanceof KeystrokeEvent)) {
        ...
        //
        if (ev instanceof SessionShutdownEvent) throw new InterruptedException();

        // Dispatch event
        //
        ev.dispatch(m_target);
        if ((flags & FLAG_STOP_ON_EVENT) != 0) throw new EventDeliveredException(ev, "", -1);
      ...
      if (m_keystrokeListener != null) m_keystrokeListener.keystroke(ch);
      return ch;
    }
  }
    ...
    while (token == null) {
      ...
      for (; ; ) {
        try {
          ...
          if ((flags & FLAG_STOP_ONLY_WHEN_EMPTY) == 0) throw e;
        }
      }
  ...
  public String editLine(String defaultString, String stopChars, int maxLength, int pos, int flags)
    ...
    if (defaultString != null) {
      ...
      if (pos != -1) {
        for (; cursorpos > pos; --cursorpos) m_out.print(BS);
      }
      m_out.flush();
    }
    ...
    while (editing) {
      ...
      switch (kind) {
        ...
        case KeystrokeTokenizerDefinition.LITERAL:
          ...
          } else buffer.insert(cursorpos++, ch);

          if ((flags & FLAG_ECHO) != 0) {
            if (ch != TAB) m_out.write(ch);
            if (cursorpos < buffer.length()) {
              m_out.write(buffer.substring(cursorpos));
              m_out.write(SPACE);
              PrintUtils.printRepeated(m_out, BS, buffer.length() - cursorpos + 1);
    ...
    //
    if ((flags & FLAG_RECORD_HISTORY) != 0
        && buffer.length() > 0
  ...
  public void shutdown() {
    if (m_eventPoller != null) m_eventPoller.interrupt();
    if (m_keystrokePoller != null) m_keystrokePoller.interrupt();
    m_eventPoller = null;
  ...
  protected synchronized Event getNextEvent() throws InterruptedException {
    while (m_eventQueue.isEmpty()) this.wait();
  ...
  public void onNewline() {
    try {
      if (m_dontCount || m_bypass) return;
      if (++m_lineCount >= m_tsProvider.getTerminalSettings().getHeight() - 1) {
        m_dontCount = true;
        if (!this.getMore(m_morePrompt)) throw new OutputInterruptedException();
        m_lineCount = 0;
      }
    } catch (InterruptedException e) {
