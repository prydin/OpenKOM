...
public class Buffer {
  ...
  public void fill(WordWrapper wrapper) {
    String line;
    while ((line = wrapper.nextLine()) != null) m_lines.add(new Line(line));
  }
  ...
  public String toString() {
    ...
    for (Iterator<Line> itor = m_lines.iterator(); itor.hasNext(); ) {
      ...
      if (content.length() > 0) {
        ...
        if (line.m_newline) buffer.append('\n');
      } else if (line.m_newline) buffer.append('\n');
    }
  ...
  public void insertBefore(int where, Line line) {
    ...
    if (where == top) m_lines.add(line);
      ...
      for (idx = 0; idx < where; ++idx) newList.add(m_lines.get(idx));
      newList.add(line);
      for (; idx < top; ++idx) newList.add(m_lines.get(idx));
      m_lines = newList;
    }
  }
  ...
  public void set(int idx, String buffer) {
    int top = m_lines.size();
    if (idx > top) throw new ArrayIndexOutOfBoundsException();
    if (idx == top) m_lines.add(new Line(buffer));
    else {
      Line line = (Line) m_lines.get(idx);
      if (line != null) line.m_content = buffer;
      else m_lines.set(idx, new Line(buffer));
    }
  }
