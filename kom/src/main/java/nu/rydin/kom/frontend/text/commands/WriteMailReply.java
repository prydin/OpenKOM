...
public class WriteMailReply extends AbstractCommand {
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    try {
      originalMessage =
          session.getMostRelevantOccurrence(session.getCurrentConferenceId(), mh.getId());
    ...
    MessageOccurrence occ =
        session.storeReplyAsMail(editor.getRecipient().getId(), msg, textNumber);
    ...
    context
        ...
        .println(
            context
                .getMessageFormatter()
                .format(
                    "write.mail.saved", session.getUser(editor.getRecipient().getId()).getName()));
  }
}
