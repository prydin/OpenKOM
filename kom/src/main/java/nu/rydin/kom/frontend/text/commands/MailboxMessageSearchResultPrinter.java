...
public class MailboxMessageSearchResultPrinter implements MessageSearchResultPrinter {
  ...
  public void printSearchResultRow(Context context, KOMWriter out, MessageSearchResult msr) {
    MessageFormatter formatter = context.getMessageFormatter();
    LocalMessageSearchResult lmsr = (LocalMessageSearchResult) msr;
    DisplayController dc = context.getDisplayController();
    dc.normal();
    PrintUtils.printRightJustified(out, String.valueOf(lmsr.getLocalId()), LOCALID_COL_WIDTH);
    out.print(" ");
    dc.output();
    String user = null;
    String direction = null;
    NameAssociation mailRecipient = lmsr.getMailRecipient();
    if (lmsr.getAuthor().getId() != context.getLoggedInUserId() || mailRecipient == null) {
    ...
    dc.output();
    PrintUtils.printLeftJustified(out, lmsr.getSubject(), SUBJECT_COL_WIDTH);
    out.println();
    dc.normal();
    out.flush();
  }
  ...
  public void printSearchResultHeader(Context context) {
    ...
    hp.addHeader(formatter.format("search.mailbox.heading.text"), LOCALID_COL_WIDTH, true);
    hp.addSpace(1);
    hp.addSpace(DIRECTION_COL_WIDTH);
    hp.addSpace(1);
    hp.addHeader(formatter.format("search.mailbox.heading.user"), USER_COL_WIDTH, false);
    hp.addSpace(3);
    hp.addHeader(formatter.format("search.mailbox.heading.subject"), SUBJECT_COL_WIDTH, false);
    hp.printOn(out);
  }
}
