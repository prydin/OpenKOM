...
public class Help extends AbstractCommand {
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    try {
      ...
      xr.parse(new InputSource(ClassLoader.getSystemClassLoader().getResourceAsStream("help.xml")));
    ...
    for (int i = 0; i < clpa.length; ++i) {
      // TODO: Sluta hota �lgbest�ndet h�r.
      //
      out.print(
          (0 < i ? new String(new char[] {clpa[i].getSeparator(), ' '}) : " ")
              + clpa[i].getUserDescription(context));
    }
    out.println();
    out.println();
    ...
    if (0 < clpa.length) {
      ...
      for (int i = 0; i < clpa.length; ++i) {
        PrintUtils.printIndented(
            out,
            clpa[i].getUserDescription(context) + ": " + (null == pDesc[i] ? "" : pDesc[i]),
            termWidth,
            "  ");
      }
      out.println();
    ...
    if (0 < myRefs.size()) {
      ...
      for (int i = 0; i < htc.getReferences().size(); ++i) {
        ...
        if (p[1].equals("command")) out.println("  " + formatter.format(p[0] + ".name"));
        else out.println("  " + p[0]);
      }
    }
  }
}
