...
public class Signup extends AbstractCommand {
  public Signup(Context context, String fullName, long permissions) {
    super(
        fullName, new CommandLineParameter[] {new ConferenceWildcardParameter(true)}, permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    //
    if (-1
        == ((NameAssociation) parameterArray[0]).getId()) // .equals(parameterArray[0].toString()))
    {
      ...
      int choice =
          in.getChoice(
              mf.format("signup.allconfs.verify")
                  + " ("
                  + mf.format("misc.y")
                  + "/"
                  + mf.format("misc.n")
                  + ")? ",
              new String[] {mf.format("misc.n"), mf.format("misc.y")},
              0,
      ...
      if (1 == choice) {
        ...
        out.println();
        out.println(
            mf.format("signup.allconfs.result", l)
                + " "
