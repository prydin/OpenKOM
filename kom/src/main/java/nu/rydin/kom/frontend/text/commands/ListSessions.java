  ...
  public void execute(Context context, Object[] parameterArray) throws KOMException {
    ...
    for (int idx = 0; idx < top; ++idx) {
      ...
      PrintUtils.printLeftJustified(
          out, formatter.format("clienttypes." + Integer.toString(each.getClientType())), 7);
      out.print(' ');
      PrintUtils.printIndented(
          out, context.formatObjectName(each.getUser()), lastColWidth, 0, firstColsWidth);
      if (idle < 60000) ++active;
    }
    out.println();
    out.println(
        formatter.format(
            "list.sessions.total", new Object[] {new Integer(top), new Integer(active)}));
  }
}
