...
public class KeystrokeTokenizerDefinition implements Cloneable {
  ...
  public KeystrokeTokenizerDefinition(String[] patterns, int[] kinds)
    ...
    for (int idx = 0; idx < top; ++idx) this.addPattern(patterns[idx], kinds[idx]);
  }
  ...
  public void addPattern(String pattern, int kind) throws AmbiguousPatternException {
    ...
    for (int idx = 0; idx < top; ++idx) {
      ...
      if (next != null) {
        ...
        //
        if (idx == top - 1) throw new AmbiguousPatternException(pattern);
      } else {
