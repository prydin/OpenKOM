...
public class EllipsisParameter extends CommandLineParameter {
  ...
  public EllipsisParameter(
      ...
      boolean isRequired,
      CommandLineParameter innerParameter,
      DefaultStrategy def) {
    this(missingObjectQuestionKey, isRequired, innerParameter, ':', def);
  }

  public EllipsisParameter(
      String missingObjectQuestionKey, boolean isRequired, CommandLineParameter innerParameter) {
  ...
  public Object resolveFoundObject(Context context, Match match)
    ...
    for (int i = 0; i < parsedObjects.length; i++) {
      ...
      result[i] =
          this.innerParameter.resolveFoundObject(context, new Match(true, parsed, "", parsed));
    }
    return result;
  }
