...
public class CompactMessagePrinter extends AbstractMessagePrinter {
  ...
  protected void printMailInformation(Context context, Envelope envelope) {
    ...
    for (int idx = 0; idx < attributes.length; ++idx) {
      ...
      if (each.getKind() == MessageAttributes.MAIL_RECIPIENT
          && each.getUserId() != context.getLoggedInUserId()) {
        PrintUtils.printIndented(
            out,
            formatter.format(
    ...
    if (reply.isLocal()) {
      ...
      return null == attrs
          ? formatter.format(
              ...
              new Object[] {
                new Long(occ.getLocalnum()), reply.getConferenceName(), reply.getAuthorName()
              })
          : formatter.format(
