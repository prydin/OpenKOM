...
public class Parser {
  ...
  public boolean removeAlias(String alias) {
    ...
    int pos = -1;
    for (int i = 0; i < top; ++i) {
      if (!(m_commands[i] instanceof AliasWrapper)) continue;
  ...
  public ExecutableCommand parseCommandLine(Context context, String commandLine)
  ...
  private CommandToMatches resolveMatchingCommand(Context context, String commandLine)
      throws IOException, InterruptedException, KOMException, CommandNotFoundException {
  ...
  private List<CommandToMatches> resolveAmbiguousCommand(
      Context context, List<CommandToMatches> potentialTargets)
      throws IOException, InterruptedException, KOMException {
  ...
  private ExecutableCommand resolveParametersForTarget(Context context, CommandToMatches target)
      throws InvalidParametersException, TooManyParametersException, IOException,
          InterruptedException, KOMException, OperationInterruptedException {
  ...
  private List<CommandToMatches> getPotentialTargets(String commandLine, Context context)
    ...
    for (Command each : m_commands) {
      if (each.hasAccess(context)) potentialTargets.add(new CommandToMatches(each));
    }
