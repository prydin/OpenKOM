...
public class SendBroadcastChatMessage extends AbstractCommand {
  ...
  public SendBroadcastChatMessage(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new RawParameter("chat.saytoall", false)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray)
    ...
    if (0 != (session.getLoggedInUser().getFlags1() & UserFlags.CONFIRM_BROADCAST_MESSAGES)) {
      ...
      int choice =
          in.getChoice(
              formatter.format("chat.confirm")
                  + " ("
                  + formatter.format("misc.y")
                  + "/"
                  + formatter.format("misc.n")
                  + ")? ",
              new String[] {formatter.format("misc.n"), formatter.format("misc.y")},
              -1,
