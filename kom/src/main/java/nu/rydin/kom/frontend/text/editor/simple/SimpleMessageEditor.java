...
public class SimpleMessageEditor extends AbstractEditor implements MessageEditor {
  public SimpleMessageEditor(Context context) throws IOException, UnexpectedException {
  ...
  public UnstoredMessage edit(
    ...
    try {
              ...
              formatter.format(
                  ...
                  new Object[] {
                    ...
                    this.formatObjectName(
                        this.getSession().getName(replyTo.getConference()),
                        replyTo.getConference()),
      ...
      dc.input();
      out.flush();
      if (askForSubject) this.setSubject(in.readLine(oldSubject).trim());
      else {
        this.setSubject(oldSubject);
        out
            .println(); // Since the user doesn't give us a line feed, we'll have to provide one
                        // ourselves
      }

      dc.messageHeader();
      PrintUtils.printRepeated(
          out, '-', Math.min(width - 1, subjLine.length() + this.getSubject().length()));
      out.println();
