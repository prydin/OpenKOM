...
public class DataAccessPool {
  ...
  public DataAccessPool() throws UnexpectedException {
    ...
    for (int idx = 0; idx < top; ++idx) m_pool.add(this.createDataAccess());
  }
  ...
  public DataAccess getDataAccess() throws UnexpectedException {
    ...
    DataAccess da = null;
    synchronized (this) {
      if (!m_pool.isEmpty()) da = (DataAccess) m_pool.removeFirst();
    }

    // Did we get anything? Is it working?
    //
    if (da == null || !da.isValid()) da = this.createDataAccess();
