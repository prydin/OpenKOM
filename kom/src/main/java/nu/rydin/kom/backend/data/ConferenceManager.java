...
public class ConferenceManager {
  ...
  public ConferenceManager(final Connection conn, final NameManager nameManager)
      throws SQLException {
    ...
    m_changeReplyToConfStmt =
        conn.prepareStatement("UPDATE conferences SET replyConf = ? WHERE id = ?");
    m_loadConfStmt =
        conn.prepareStatement(
    ...
    m_isMailboxStmt = conn.prepareStatement("SELECT COUNT(*) FROM users WHERE id = ?");
    m_listByDateStmt =
        conn.prepareStatement(
    ...
    m_changePermissionsStmt =
        conn.prepareStatement(
            "UPDATE conferences SET permissions = ?, nonmember_permissions = ? WHERE id = ?");
    m_getParentCountForConfStmt =
        conn.prepareStatement("select count(*) from conferences where replyConf = ?");
    m_getMaxParentCountStmt =
        conn.prepareStatement(
            "select count(*) as count from conferences where replyConf is not null group by replyConf order by count desc limit 1");
  }
  ...
  public long addConference(
      ...
      final long administrator,
      final int permissions,
      final int nonmemberPermissions,
      final short visibility,
      final long replyConf)
      throws DuplicateNameException, SQLException, UnexpectedException {
    ...
    //
    final long nameId =
        m_nameManager.addName(fullname, NameManager.CONFERENCE_KIND, visibility, keywords);
  ...
  public void changeReplyToConference(
      final long originalConferenceId, final long newReplyToConferenceId) throws SQLException {
   ...
   * @throws SQLException
   */
  public void addMailbox(final long user, final int permissions) throws SQLException {
  ...
  public ConferenceInfo loadConference(final long id) throws ObjectNotFoundException, SQLException {
    final KOMCache cache = CacheManager.instance().getConferenceCache();
    final Long key = id;
    final ConferenceInfo cached = (ConferenceInfo) cache.get(key);
    if (cached != null) {
      return cached;
    }
    m_loadConfStmt.clearParameters();
    m_loadConfStmt.setLong(1, id);
    try (final ResultSet rs = m_loadConfStmt.executeQuery()) {
      if (!rs.next()) {
      ...
      //
      final MessageRange r = getMessageRange(id);
      final ConferenceInfo answer =
          new ConferenceInfo(
              id, // Id
  ...
  public MessageRange getMessageRange(final long id) throws ObjectNotFoundException, SQLException {
    m_loadRangeStmt.clearParameters();
    m_loadRangeStmt.setLong(1, id);
    try (final ResultSet rs = m_loadRangeStmt.executeQuery()) {
      if (!rs.next()) {
        throw new ObjectNotFoundException("Conference id=" + id);
      }
   ...
   * @throws SQLException
   */
  public long[] getConferenceIdsByPattern(final String pattern) throws SQLException {
    return m_nameManager.getIdsByPatternAndKind(pattern, NameManager.CONFERENCE_KIND);
  }

  public boolean isMailbox(final long conference) throws SQLException, ObjectNotFoundException {
    ResultSet rs = null;
    try {
  ...
  public ConferenceListItem[] listByDate(final long user) throws SQLException {
    m_listByDateStmt.clearParameters();
    m_listByDateStmt.setLong(1, user);
    ResultSet rs = null;
    try {
      final ArrayList<ConferenceListItem> list = new ArrayList<>();
      ...
      while (rs.next()) {
        ...
        list.add(
            new ConferenceListItem(
                id,
                new Name(rs.getString(2), rs.getShort(3), NameManager.CONFERENCE_KIND),
                rs.getTimestamp(4),
                rs.getTimestamp(5),
      ...
      final ConferenceListItem[] answer = new ConferenceListItem[list.size()];
      list.toArray(answer);
      return answer;
    } finally {
      if (null != rs) {
  ...
  public long countParentsForConference(final long conf) throws SQLException {
    ResultSet rs = null;
    try {
  ...
  public ConferenceListItem[] listByName(final long user) throws SQLException {
