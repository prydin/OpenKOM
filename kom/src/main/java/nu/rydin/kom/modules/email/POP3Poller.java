...
public class POP3Poller extends Thread {
  ...
  public POP3Poller(
      ...
      int pollDelay,
      String deadLetterArea,
      long systemMessageConf) {
    this.host = host;
    this.port = port;
    this.user = user;
            ...
            if (top > 0) {
              ...
              try {
                for (int idx = 0; idx < top; ++idx) {
                  ...
                  if (recipients == null) {
                    this.handleDeadLetter(
                        each, "Null recipients. Can't handle message from " + each.getFrom(), ss);
                    continue;
                  }
                  for (int idx2 = 0; idx2 < recipients.length; ++idx2) {
                    ...
                    try {
                      ...
                      if (ss == null)
                        ss =
                            ssf.login(
                                this.postmaster, this.postmasterPassword, ClientTypes.SOAP, true);
  ...
  protected void handleDeadLetter(Message message, String reason, ServerSession ss)
