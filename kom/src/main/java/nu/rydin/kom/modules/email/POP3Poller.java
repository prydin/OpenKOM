...
public class POP3Poller extends Thread {
  ...
  public void run() {
    ...
    try {
      ...
      LOG.fatal("Backend not running! Can't start postmaster");
      return;
    }
    LOG.info("Polling mailbox " + user + " at host " + host);
    try {
      for (; ; ) {
        ...
        try {
          ...
          try {
              ...
              try {
                for (final Message each : messages) {
                  ...
                  for (final Address recipient : recipients) {
                    ...
                    LOG.info("Processing email to " + to);
                    ...
                    try {
                      ...
                      LOG.error("Internal error when processing email", e);
            ...
            LOG.error("Login prohibited. Trying again...");
          } catch (final AuthenticationException e) {
            LOG.error("Cannot log in postmaster. Wrong user/password. Trying again...");
          } catch (final AlreadyLoggedInException e) {
            LOG.error("Congratulations! This can't happen!");
          } finally {
            folder.close(commit);
            store.close();
          }
