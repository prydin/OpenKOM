...
public class MessageFormatter {
  ...
  public MessageFormat getFormat(String key) {
    ...
    MessageFormat mf = new MessageFormat(fmt != null ? fmt : "Unknown resource: " + key, m_locale);
    Format[] subformats = mf.getFormats();
    for (int idx = 0; idx < subformats.length; idx++) {
      Format each = subformats[idx];
      if (each instanceof SimpleDateFormat) ((SimpleDateFormat) each).setTimeZone(m_timeZone);
    }
    return mf;
  }
}
