/**
 * FreeBSD-compatible md5-style password crypt, based on crypt-md5.c by Poul-Henning Kamp, which was
 * distributed with the following notice:
 *
 * <p>---------------------------------------------------------------------------- "THE BEER-WARE
 * LICENSE" (Revision 42): <phk@login.dknet.dk> wrote this file. As long as you retain this notice
...
public class MD5Crypt {
  ...
  private static String cryptTo64(int value, int length) {
    ...
    while (--length >= 0) {
      output.append(MD5Crypt.itoa64.substring(value & 0x3f, (value & 0x3f) + 1));
      value >>= 6;
    }
   ...
   * @throws java.security.NoSuchAlgorithmException if java.security does not support MD5
  ...
  public static String crypt(String password, String salt)
    ...
    if (salt.startsWith(MD5Crypt.magic)) {
      salt = salt.substring(MD5Crypt.magic.length());
    }
    ...
    md5_1.update(MD5Crypt.magic.getBytes());
    ...
    final int md5Size = md5_2_digest.length; // XXX
     ...
     */
    final StringBuffer output = new StringBuffer(MD5Crypt.magic);
    output.append(salt);
    output.append("$");
    ...
    int value;
    value =
        ((md5_1_digest[0] & 0xff) << 16)
            | ((md5_1_digest[6] & 0xff) << 8)
            | (md5_1_digest[12] & 0xff);
    output.append(MD5Crypt.cryptTo64(value, 4));
    value =
        ((md5_1_digest[1] & 0xff) << 16)
            | ((md5_1_digest[7] & 0xff) << 8)
            | (md5_1_digest[13] & 0xff);
    output.append(MD5Crypt.cryptTo64(value, 4));
    value =
        ((md5_1_digest[2] & 0xff) << 16)
            | ((md5_1_digest[8] & 0xff) << 8)
            | (md5_1_digest[14] & 0xff);
    output.append(MD5Crypt.cryptTo64(value, 4));
    value =
        ((md5_1_digest[3] & 0xff) << 16)
            | ((md5_1_digest[9] & 0xff) << 8)
            | (md5_1_digest[15] & 0xff);
    output.append(MD5Crypt.cryptTo64(value, 4));
    value =
        ((md5_1_digest[4] & 0xff) << 16)
            | ((md5_1_digest[10] & 0xff) << 8)
            | (md5_1_digest[5] & 0xff);
    output.append(MD5Crypt.cryptTo64(value, 4));
    value = md5_1_digest[11] & 0xff;
    output.append(MD5Crypt.cryptTo64(value, 2));
