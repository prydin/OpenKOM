...
public class CreateFilter extends AbstractCommand {
  public CreateFilter(Context context, String fullName, long permissions) {
    super(
        ...
        new CommandLineParameter[] {
          ...
          new EllipsisParameter(
              "create.filter.type.ask",
              true,
              new FlagParameter(true, context.getFlagLabels("filterflags")))
        },
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameters)
    ...
    long flags = 0;
    for (int i = 0; i < flagList.length; i++) flags |= 1 << ((Integer) flagList[i]).intValue();
