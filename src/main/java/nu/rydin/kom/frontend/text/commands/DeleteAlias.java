...
public class DeleteAlias extends AbstractCommand {
  public DeleteAlias(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new StringParameter("delete.alias.param.ask.0", true)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameters)
    ...
    int choice =
        in.getChoice(
            mf.format("delete.alias.verify", alias)
                + " ("
                + mf.format("misc.y")
                + "/"
                + mf.format("misc.n")
    ...
    if (1 == choice) {
      ...
      else out.println(mf.format("delete.alias.no.such.alias", alias));
      return;
    } else {
