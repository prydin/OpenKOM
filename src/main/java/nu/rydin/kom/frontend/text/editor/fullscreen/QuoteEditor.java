...
public class QuoteEditor extends FullscreenEditor {
  ...
  public QuoteEditor(Context context, MessageLocator replyTo, FullscreenMessageEditor mainEditor)
      throws IOException, ObjectNotFoundException, AuthorizationException, UnexpectedException,
          NoCurrentMessageException, EmptyMessageException {
    super(context);
    this.mainEditor = mainEditor;
    ServerSession session = context.getSession();
    String body = session.readMessage(replyTo).getMessage().getBody();
    WordWrapper ww =
        context.getWordWrapper(body, QUOTE_WIDTH - 8); // 8 = length of ">" + linenumber.
    ...
    if (buffer.size() == 0) throw new EmptyMessageException();
  }
