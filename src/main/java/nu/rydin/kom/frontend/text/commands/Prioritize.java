...
public class Prioritize extends AbstractCommand {
  public Prioritize(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new ConferenceParameter(true), new ConferenceParameter(false)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray) throws KOMException {
    ...
    if (parameterArray[1] == null) {
      ...
      targetConference =
          context.getSession().listMemberships(context.getLoggedInUserId())[0].getId();
    } else {
    ...
    out.println(
        fmt.format(
            ...
            new Object[] {
              new Long(shuffled),
              context.formatObjectName((NameAssociation) parameterArray[0]),
              new Long(Math.abs(shuffled))
            }));
  }
}
