...
public class FindObject extends AbstractCommand {
  public FindObject(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new RawParameter("find.object.ask.0", true)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameters)
    ...
    if (top > 0) {
      ...
      out.println();
      dc.output();
      for (int idx = 0; idx < top; ++idx) out.println(context.formatObjectName(hits[idx]));
    } else out.println(fmt.format("find.object.no.match"));
    dc.normal();
  }
}
