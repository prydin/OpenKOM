...
public class BasicMessagePrinter extends AbstractMessagePrinter {
  protected void printHeaderReceivers(Context context, Envelope envelope)
    ...
    for (int idx = 0; idx < receivers.length; ++idx) {
      ...
      if (context
          .getSession()
          .hasPermissionInConference(occ.getConference(), ConferencePermissions.READ_PERMISSION)) {
        String conferenceName;
        if (occ.getKind() == MessageManager.ACTION_CREATED
            && receivers[idx].getId() == context.getLoggedInUserId()) {
        ...
        PrintUtils.printIndented(
            out, formatter.format(getResourceKey("receiver"), conferenceName), width, 0);
        switch (occ.getKind()) {
          case MessageManager.ACTION_COPIED:
            PrintUtils.printIndented(
                out,
                formatter.format(
                    getResourceKey("copied"),
                    new Object[] {
  ...
  protected String getFormattedOriginInfo(
      Context context, Envelope.RelatedMessage replyTo, MessageAttribute[] attrs) {
    MessageFormatter formatter = context.getMessageFormatter();
    ...
    if (replyTo.isLocal()) {
      ...
      return attrs == null
          ? formatter.format(
              getResourceKey("reply.to.same.conference"),
              new Object[] {
                new Long(replyTo.getOccurrence().getLocalnum()),
                context.formatObjectName(replyTo.getAuthorName(), replyTo.getAuthor())
              })
          : formatter.format(
              getResourceKey("reply.to.same.conference.type"),
              new Object[] {
                new String(attrs[0].getValue()),
                new Long(replyTo.getOccurrence().getLocalnum()),
                context.formatObjectName(replyTo.getAuthorName(), replyTo.getAuthor())
              });
    } else {
      ...
      return attrs == null
          ? formatter.format(
              getResourceKey("reply.to.different.conference"),
              new Object[] {
                new Long(replyTo.getOccurrence().getLocalnum()),
          ...
          : formatter.format(
              getResourceKey("reply.to.different.conference.type"),
              new Object[] {
                new String(attrs[0].getValue()),
                new Long(replyTo.getOccurrence().getLocalnum()),
                new Long(replyTo.getOccurrence().getGlobalId()),
  ...
  protected String getFormattedOriginDeletedInfo(
      Context context, MessageAttribute originalDeletedAttribute) {
    return context
        .getMessageFormatter()
        .format(
            getResourceKey("reply.to.deleted.text"),
            context.formatObjectName(
                originalDeletedAttribute.getUsername(), originalDeletedAttribute.getUserId()));
  }
    ...
    for (int idx = 0; idx < attributes.length; ++idx) {
      ...
      switch (each.getKind()) {
        case MessageAttributes.MAIL_RECIPIENT:
          if (each.getUserId() != context.getLoggedInUserId())
            PrintUtils.printIndented(
                out,
                formatter.format(
                    getResourceKey("original.mail.recipient"),
        ...
        case MessageAttributes.EMAIL_SENDER:
          PrintUtils.printIndented(
              out, formatter.format(getResourceKey("email.sender"), each.getValue()), width, 0);
          break;
        case MessageAttributes.EMAIL_SENT:
          PrintUtils.printIndented(
              out,
              formatter.format(
                  getResourceKey("email.sent"),
                  context.smartFormatDate(new Timestamp(Long.valueOf(each.getValue())))),
              width,
              0);
          break;
  ...
  protected String getFormattedReply(
      Context context,
      Envelope.RelatedMessage reply,
    ...
    if (reply.isLocal()) {
      return null == attrs
          ? formatter.format(
              getResourceKey("reply.same.conference"),
              new Object[] {new Long(occ.getLocalnum()), reply.getAuthorName()})
          : formatter.format(
              getResourceKey("reply.same.conference.type"),
              new Object[] {
                new String(attrs[0].getValue()), new Long(occ.getLocalnum()), reply.getAuthorName()
              });
    } else {
      return null == attrs
          ? formatter.format(
              getResourceKey("reply.different.conference"),
              new Object[] {
                new Long(occ.getLocalnum()),
                new Long(occ.getGlobalId()),
                reply.getAuthorName(),
                reply.getConferenceName()
              })
          : formatter.format(
              getResourceKey("reply.different.conference.type"),
              new Object[] {
                new String(attrs[0].getValue()),
                new Long(occ.getLocalnum()),
                new Long(occ.getGlobalId()),
