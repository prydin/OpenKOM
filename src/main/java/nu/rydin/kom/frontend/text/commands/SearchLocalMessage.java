...
public class SearchLocalMessage extends SearchLocalCommand {
  public SearchLocalMessage(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new RawParameter("search.param.0.ask", true)},
        permissions);
  }

  protected MessageSearchResult[] innerSearch(Context context, Object[] parameterArray, int offset)
    ...
    LocalMessageSearchResult[] lmsr =
        context
            ...
            .searchMessagesLocally(
                context.getSession().getCurrentConferenceId(), searchterm, offset, CHUNK_SIZE);
    return lmsr;
  }
  ...
  long count(Context context, Object[] parameterArray) throws KOMException {
    ...
    return context
        .getSession()
        .countSearchMessagesLocally(context.getSession().getCurrentConferenceId(), searchterm);
  }
}
