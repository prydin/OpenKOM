...
public class ChangeTerminalHeight extends AbstractCommand {
  ...
  public ChangeTerminalHeight(Context context, String fullName, long permissions) {
    super(
        fullName,
        new CommandLineParameter[] {new IntegerParameter("change.height.param.0.ask", true)},
        permissions);
  }
  ...
  public void execute(Context context, Object[] parameterArray) throws KOMException, IOException {
    ...
    if (height == 0) {
      ...
      out.println(
          mf.format("change.terminal.info")
              + context.getTerminalSettings().getWidth()
              + "x"
              + context.getTerminalSettings().getHeight());
    } else {
