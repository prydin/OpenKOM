...
public class AuthenticationServer implements Module, Runnable {
  ...
  public void start(final Map<String, String> parameters) {
    ...
    try {
      ...
      // out of here!
      //
      AuthenticationServer.LOG.fatal(e);
      return;
    }
    AuthenticationServer.LOG.info(
        "OpenKOM authentication server is accepting connections at port " + port);
  ...
  private static class AuthenticationWorker extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        try {
          final String ticket =
              ServerSessionFactoryImpl.getInstance().generateTicket(user, password);
          out.print("OK:");
          out.print(ticket);
          out.print("\r\n");
        ...
        //
        AuthenticationServer.LOG.error(e);
      } finally {
        try {
          m_sock.close();
        } catch (final IOException e) {
          AuthenticationServer.LOG.error("Error closing socket", e);
        }
      }
    }
  }
